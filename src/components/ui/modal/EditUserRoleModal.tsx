...
export default function EditUserRoleModal({
    ...
    const handleSave = async () => {
        if (selectedUserClubRelation) {
            try {
                setLoading(true);
                const responseOk = await updateUserRole(
            ...
            <div className="text-center">
                ...
                <p className="leading-6 text-gray-500 dark:text-gray-400">
                    ...
                    {t.rich("description", {
                        ...
                        name:
                            selectedUserClubRelation?.user.displayName ||
                            t("user"),
                    })}
                </p>
                <div className="relative mt-6 flex items-center justify-center gap-3">
                    <button
                        ...
                        className={`inline-flex items-center px-2.5 py-0.5 justify-center gap-1 rounded-full font-bold ${role === "USER" ? roleColors[role] : defaultStyle}`}
                    >
                        USER
                    </button>
                    <button
                        ...
                        className={`inline-flex items-center px-2.5 py-0.5 justify-center gap-1 rounded-full font-bold ${role === "SUPERVISOR" ? roleColors[role] : defaultStyle}`}
                    >
                        SUPERVISOR
                    </button>
                    <button
                        ...
                        className={`inline-flex items-center px-2.5 py-0.5 justify-center gap-1 rounded-full font-bold ${role === "ADMIN" ? roleColors[role] : defaultStyle}`}
                    >
                        ADMIN
                    </button>
                </div>
